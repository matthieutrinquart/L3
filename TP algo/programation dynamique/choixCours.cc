...
int getNearestPred(int n, crs cours[], crs wanted) {
    for ( int i = n; i > -1 ; i-- ) {
        if ( wanted.deb > cours[i].fin ) {
            return i;
        }
    }
...
void  CalculDernierPred(int n,crs cours[], int predMax[]){
    TrierCours(n, cours);
    for ( int i = 0 ; i < n ; i++ ) {
        predMax[i] = getNearestPred(n, cours, cours[i]);
    }
}
...
void choixMaxProgD(int n, crs cours[], int predMax[], int crsMax[]){
  ...
  for (int i = 0; i < n; i++)
  {
    if(predMax[i] != -1){
      ...
      crsMax[i] = max(crsMax[i] , cours[i].val + crsMax[predMax[i]]);
    }
    else
    {
      crsMax[i] = max(crsMax[i],cours[i].val);
    }
